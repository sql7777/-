...
class SendDataToDestinationBLServer(object):
    def __init__(self, bluetuuid="94f39d29-7d6d-437d-973b-fba39e49d4ee",name="FireFly-3288C",port=6):
    ...
    def EPCSendRoServerForBluetooth(self):
        ...
        advertise_service( server_sock, self.bluetoothname,
                           service_id = uuid,
        ...
        self.thread_read=threading.Thread(target=self.RecvThreading,args=(client_sock,))
    ...
    def RunAll_Bluetooth(self):
        if self.allthread!=None:
            return False
        self.allthread=threading.Thread(target=self.EPCSendRoClientForBluetoothRUN)
...
def RunMain():
    test=SendDataToDestinationBLServer()
    test.EPCSendRoClientForBluetoothRUN()
