...
thread_read_flg=False
thread_read=None
def RecvThreading(s):
    global thread_read_flg
    while thread_read_flg:
        data = s.recv(1024)
        if len(data) == 0:
            continue#break
        ...
        if b'START' in data:
            ...
            p,var = data.split(b':')
            SetSysBluetoothCMD(cmd=int(var))
        else:
            pass
...
def EPCSendRoServerForBluetooth():
    ...
    try:
        ...
        thread_read_flg=True
        thread_read=threading.Thread(target=RecvThreading,args=(sock,))
